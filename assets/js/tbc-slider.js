...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.slider__offers__section, .slider__products__section, .slider__awards__section')
        .forEach((section) => {
            ...
            const blueLineWrapper = section.querySelector('.slider__blue__line__wrapper');
            ...
            const events = ['mousedown', 'touchstart', 'mousemove', 'touchmove', 'mouseup', 'mouseleave', 'touchend'];

            events.forEach(eventType => {
                if (eventType === 'mousedown' || eventType === 'touchstart') {
                    container.addEventListener(eventType, startDrag);
                } else if (eventType === 'mousemove' || eventType === 'touchmove') {
                    container.addEventListener(eventType, duringDrag);
                } else {
                    container.addEventListener(eventType, stopDrag);
                }
            });

            blueLine.style.width = `${(blueLineWrapper.clientWidth / container.scrollWidth) * 100}%`;
            updateButtons();
            moveBlueLine();
        });
});
